...
layer1234_start:
		...
		// 8-NTT on a1, a3, ..., a15
		_3_layer_double_CT_16_plant_fp R<poly1>, R<poly3>, R<poly5>, R<poly7>, R<poly9>, R<poly11>, R<poly13>, R<poly15>, s8, s9, s10, s11, s12, s13, s14, twiddle1, twiddle2, q, qa, tmp

		// s15, s16, s17, s18, s19, s20, s21, s22 left
		// multiply coeffs by layer 8 twiddles for later use
		vmov twiddle1, s15 
		vmov twiddle2, s16 
		mul_twiddle_plant R<poly1>, twiddle1, tmp, q, qa
		mul_twiddle_plant R<poly3>, twiddle2, tmp, q, qa

		vmov twiddle1, s17 
		vmov twiddle2, s18 
		mul_twiddle_plant R<poly5>, twiddle1, tmp, q, qa
		mul_twiddle_plant R<poly7>, twiddle2, tmp, q, qa

		vmov twiddle1, s19 
		vmov twiddle2, s20 
		mul_twiddle_plant R<poly9>, twiddle1, tmp, q, qa
		mul_twiddle_plant R<poly11>, twiddle2, tmp, q, qa

		vmov twiddle1, s21 
		vmov twiddle2, s22 
		mul_twiddle_plant R<poly13>, twiddle1, tmp, q, qa
		mul_twiddle_plant R<poly15>, twiddle2, tmp, q, qa
		...
		//vmov s3, R<poly7> // a7
		//vmov s4, R<poly9> // a9
		//vmov s5, R<poly11> // a11
		//vmov s6, R<poly13> // a13
		//vmov s7, R<poly15> // a15
		...
		movw qa, #26632
		// 8-NTT on a0, a2, ..., a14
		_3_layer_double_CT_16_plant_fp R<poly0>, R<poly2>, R<poly4>, R<poly6>, R<poly8>, R<poly10>, R<poly12>, R<poly14>, s8, s9, s10, s11, s12, s13, s14, twiddle1, twiddle2, q, qa, tmp

		
		// layer 4 - 1
		// addsub: (a2, a6, a10, a14), (a3, a7, a11, a15)
		vmov poly, s23
		//vmov R<poly3>, s1 // load a3
		uadd16 tmp, R<poly2>, R<poly3>
		usub16 R<poly2>, R<poly2>, R<poly3>
		str.w tmp, [poly, #1*distance/4]
		str.w R<poly2>, [poly, #1*distance/4+offset]

		//vmov R<poly7>, s3 // load a7
		uadd16 tmp, R<poly6>, R<poly7>
		usub16 R<poly6>, R<poly6>, R<poly7>
		str.w tmp, [poly, #3*distance/4]
		str.w R<poly6>, [poly, #3*distance/4+offset]
		
		//vmov R<poly11>, s5 // load a11
		uadd16 tmp, R<poly10>, R<poly11>
		usub16 R<poly10>, R<poly10>, R<poly11>
		str.w tmp, [poly, #5*distance/4]
		str.w R<poly10>, [poly, #5*distance/4+offset]
		
		//vmov R<poly15>, s7 // load a15
		uadd16 tmp, R<poly14>, R<poly15>
		usub16 R<poly14>, R<poly14>, R<poly15>
		str.w tmp, [poly, #7*distance/4]
		str.w R<poly14>, [poly, #7*distance/4+offset]
		
		// layer 4 - 2    
		// addsub: (a0, a4, a8, a12), (a1, a5, a9, a13)
		//vmov R<poly5>, s2 // load a5
		uadd16 tmp, R<poly4>, R<poly5>
		usub16 twiddle1, R<poly4>, R<poly5>
		str.w tmp, [poly, #2*distance/4]
		str.w twiddle1, [poly, #2*distance/4+offset]

		//vmov R<poly9>, s4 // load a9
		uadd16 tmp, R<poly8>, R<poly9>
		usub16 twiddle1, R<poly8>, R<poly9>
		str.w tmp, [poly, #4*distance/4]
		str.w twiddle1, [poly, #4*distance/4+offset]

		//vmov R<poly13>, s6 // load a13
		uadd16 tmp, R<poly12>, R<poly13>
		usub16 twiddle1, R<poly12>, R<poly13>
		str.w tmp, [poly, #6*distance/4]
		str.w twiddle1, [poly, #6*distance/4+offset]
		
		//vmov R<poly1>, s0 // load a1
		uadd16 tmp, R<poly0>, R<poly1>
		usub16 twiddle1, R<poly0>, R<poly1>
